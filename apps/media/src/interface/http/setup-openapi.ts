...
/**
 * Publishes the media OpenAPI spec at `/api/v1/openapi.json` and a Scalar
...
export function setupOpenApi(app: INestApplication): void {
  const config = new DocumentBuilder()
    ...
    .setDescription(
      'Presigned direct-to-storage image uploads + thumbnail generation. The app never proxies bytes.',
    )
    .setVersion('1.0')
    .addBearerAuth()
    .build();
