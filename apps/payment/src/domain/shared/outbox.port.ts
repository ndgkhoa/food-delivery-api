 ...
 *
 * Tenant is stamped by the adapter from the consume-time context — never from
 * the entry. `correlationId` carries the triggering command's saga-wide trace id
 * onto the reply so the whole saga shares one id; the adapter mints one only when
 * it is absent (the header must be non-null).
 */
