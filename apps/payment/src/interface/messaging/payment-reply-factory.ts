...
export function paymentSucceededReply(orderId: string): OutboxCommandEntry {
  return {
    aggregateId: orderId,
...
export function paymentFailedReply(orderId: string, reason: string): OutboxCommandEntry {
  return {
    aggregateId: orderId,
    topic: PAYMENT_REPLIES_TOPIC,
