...
import type { InventoryOutboxOrmEntity } from '@inventory/infrastructure/persistence/entities/inventory-outbox.orm-entity';
...
class FakeOutboxRepository {
  readonly saved: InventoryOutboxOrmEntity[] = [];
  create(row: Partial<InventoryOutboxOrmEntity>): InventoryOutboxOrmEntity {
    return row as InventoryOutboxOrmEntity;
  }
  async save(row: InventoryOutboxOrmEntity): Promise<InventoryOutboxOrmEntity> {
    this.saved.push(row);
    return row;
  }
}
...
describe('TypeOrmInventoryOutboxAdapter', () => {
  beforeEach(() => {
  ...
  describe('append', () => {
    it('persists the captured traceparent when a trace context is active', async () => {
      ...
      const adapter = new TypeOrmInventoryOutboxAdapter(
        fallback as unknown as Repository<InventoryOutboxOrmEntity>,
        fakeDataSource([]),
        tenantContext,
      );
    ...
    it('enlists the active transactional entity manager instead of the default repository', async () => {
      ...
      const manager = {
        getRepository: () => txRepo as unknown as Repository<InventoryOutboxOrmEntity>,
      } as unknown as EntityManager;
  ...
  describe('fetchUnpublished', () => {
    ...
    it('puts the persisted traceparent into the mapped headers', async () => {
      mockCaptureActiveTraceContext.mockReturnValue({});
      const adapter = new TypeOrmInventoryOutboxAdapter(
        new FakeOutboxRepository() as unknown as Repository<InventoryOutboxOrmEntity>,
        fakeDataSource([
  ...
  describe('incrementAttempts', () => {
    it('skips the update call when no ids are given', async () => {
      ...
      const adapter = new TypeOrmInventoryOutboxAdapter(
        outboxRepository as unknown as Repository<InventoryOutboxOrmEntity>,
        fakeDataSource([]),
        tenantContext,
      );
