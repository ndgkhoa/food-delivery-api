...
- [ ] E2E: submit rating → restaurant avg changes → search ranks/filters by rating; one-review-per-order + non-owner rejection — specs written (`apps/review-e2e`, gated `RUN_REVIEW_E2E`/`RUN_REVIEW_SEARCH_E2E`), **needs live verification** (not run by the implementer per environment constraints)
