...
- **Status**: ✅ Verified + reviewed + fixed — payment now a durable Temporal workflow, proven live against a real Temporal server (`workflow` compose profile: auto-setup 1.29.7 + dedicated Postgres + UI :8233). The worker registers + polls task queue `payment-charges`; `ChargePayment` → `chargeWorkflow` (workflowId `charge-{orderId}`) → charge activity → emit-reply activity → `payment_outbox` → `payment.replies` (UNCHANGED saga contract). payment-e2e **3/3** (happy → PaymentSucceeded, decline → PaymentFailed, redelivery → one reply); Temporal workflow specs **4/4** via `TestWorkflowEnvironment` (charge→reply, decline, async-pending reconcile via signal, sync-no-wait); payment unit **25** pass; offline gates clean (cruiser 91-mod/knip/biome/tsc). The `workflowsPath` webpack-bundle deviation works under `nx serve`. One e2e scenario left `todo` — an automated SIGKILL-mid-charge test; durability is proven by the replay-safe determinism tests + is UI-demonstrable. Branch `feat/payment-temporal-workflow` (PR #18).
- **Adversarial review + fixes applied** (report `../reports/code-reviewer-260729-0055-slice-5a-payment-temporal-red-team-review-report.md`; determinism + exactly-once reply verified airtight):
  - **C1 (Critical)** — workflow-id idempotency was weaker than claimed: `client.start` set no reuse policy → Temporal defaults to ALLOW_DUPLICATE, so `AlreadyStarted` only fired while the run was OPEN; a command redelivered after the workflow CLOSED (Kafka commits post-handler) started a NEW run that re-ran the charge activity (a real PSP would double-charge). **Fixed**: `workflowIdReusePolicy: 'REJECT_DUPLICATE'` — one charge per order id for the whole retention window.
  - **H1** — the HMAC webhook secret carried a public repo default (`getOrThrow` never threw → prod could run a forgeable-callback secret). **Fixed**: schema `superRefine` fails boot when `NODE_ENV=production` still holds the dev sentinel (dev default kept for convenience; unit-tested).
  - **H2** — a fixed 2s `condition` window taxed EVERY charge with latency. **Fixed**: the reconcile wait is entered ONLY when the charge activity reports `pending` (a real async PSP); the synchronous stub replies immediately (no timer).
  - **H3** — a late webhook signalling a CLOSED workflow threw → HTTP 500 the provider retries. **Fixed**: `signalProviderResult` swallows `WorkflowNotFoundError` → a post-completion callback is an accepted no-op (the reply was already emitted).
  - Deferred (documented, not blocking the learning slice): **M1** webhook nonce (only a ±300s replay window today), **M3** hard Temporal boot dependency, **M4** cwd-relative `workflowsPath`, **L1** cruiser rules cover direct-import+denylist only, **L2** webhook not tenant-scoped. Real-provider hardening (nonce + widened async reconcile) rides the Stripe activity swap.
